$(function() {
	...
	// 由于商品添加和编辑使用的是同一个页面，设定idEdit标识符用来表明本次是添加操作还是编辑操作
	var isEdit = false;
	if (productId) {
		// 若url中含有productId则为编辑商品操作
		getInfo(productId);
		isEdit = true;
	} else {
		// 否则进入添加商品操作
	...
	function getInfo(id) {
		$
				.getJSON(
						...
						function(data) {
							if (data.success) {
								// 从返回JSON当中获取product对象的信息，并复制给表单
	...
	// 若商品详情图控件组的最后一个元素发生了变化(上传图片)，且控件总数未达到6个，则生成一个新的文件上传控件
				...
				$('.detail-img').map(
						function(index, item) {
							...
							if ($('.detail-img')[index].files.length > 0) {
								// 将第i个文件赋值给key为productImgi的表单键值对
								formData.append('productImg' + index,
										$('.detail-img')[index].files[0]);
							}
						});
				// 将product json对象转化成字符流保存至表单对象key为productStr的键值对里
